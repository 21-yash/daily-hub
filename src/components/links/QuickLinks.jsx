...
import { ExternalLink, Plus, Trash2, Edit2, Star, X } from 'lucide-react';
...
const QuickLinks = ({ theme, links, setLinks }) => {
  ...
  React.useEffect(() => {
    if (links.length === 0) {
      setLinks([
        { id: 'default_1', title: 'Google', url: 'https://www.google.com', category: 'Search', favourite: true },
        { id: 'default_2', title: 'GitHub', url: 'https://github.com', category: 'Development', favourite: true },
        { id: 'default_3', title: 'Stack Overflow', url: 'https://stackoverflow.com', category: 'Development', favourite: false },
        { id: 'default_4', title: 'YouTube', url: 'https://www.youtube.com', category: 'Social', favourite: true },
        { id: 'default_5', title: 'Gmail', url: 'https://mail.google.com', category: 'Productivity', favourite: true },
        { id: 'default_6', title: 'LinkedIn', url: 'https://www.linkedin.com', category: 'Social', favourite: false },
        { id: 'default_7', title: 'Twitter', url: 'https://twitter.com', category: 'Social', favourite: false },
        { id: 'default_8', title: 'Replit', url: 'https://replit.com', category: 'Development', favourite: true },
        { id: 'default_9', title: 'MDN Web Docs', url: 'https://developer.mozilla.org', category: 'Development', favourite: false },
        { id: 'default_10', title: 'ChatGPT', url: 'https://chat.openai.com', category: 'AI', favourite: true },
        { id: 'default_11', title: 'Netflix', url: 'https://www.netflix.com', category: 'Entertainment', favourite: false },
        { id: 'default_12', title: 'Amazon', url: 'https://www.amazon.in', category: 'Shopping', favourite: false },
        { id: 'default_13', title: 'Flipkart', url: 'https://www.flipkart.com', category: 'Shopping', favourite: false },
        { id: 'default_14', title: 'WhatsApp Web', url: 'https://web.whatsapp.com', category: 'Social', favourite: true },
      ]);
    }
  }, []);
  ...
  const addLink = () => {
    ...
    if (editingLink) {
      ...
      if (authService.isAuthenticated()) {
        syncQueue.add('link', 'update', updatedLink, editingLink.id);
      }
    } else {
      ...
      const optimisticLink = { ...link, id: tempId };
      setLinks([optimisticLink, ...links]);
      
      if (authService.isAuthenticated()) {
        syncQueue.add('link', 'create', link, tempId);
      }
    }
  ...
  const deleteLink = (id) => {
    ...
    if (authService.isAuthenticated() && !id.toString().startsWith('temp_') && !id.toString().startsWith('default_')) {
      syncQueue.add('link', 'delete', { id }, id);
    }
  };
  ...
  const togglefavourite = (id) => {
    ...
    if (authService.isAuthenticated() && !id.toString().startsWith('default_')) {
      syncQueue.add('link', 'update', updatedLink, id);
    }
  };
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          >
            {showAddLink ? <X size={20} /> : <Plus size={20} />}
          </button>
        </div>
      </div>
      <select
          ...
          <div key={link.id} className={`${cardBg} p-4 rounded-xl border ${borderColor} transition-all duration-200 hover:shadow-lg hover:scale-105 relative group`}>
            <button
              ...
              className={`absolute top-2 right-2 p-1 rounded transition-all duration-200 ${
                link.favourite ? 'text-yellow-500' : 'text-gray-400 hover:text-yellow-500'
              }`}
            >
              <Star size={20} fill={link.favourite ? 'currentColor' : 'none'} />
            </button>
